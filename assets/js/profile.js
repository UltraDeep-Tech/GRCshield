document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateProfile(event) {
        ...
        const fields = [
            'fullName', 'email', 'company', 'job', 'country', 
            'address', 'phone', 'linkedin', 'about'
        ];

        fields.forEach(field => {
            const value = document.querySelector(`#${field}Edit`).value.trim();
            if (value) {
        ...
        .catch(error => {
            console.error('Error:', error);
            alert('Ocurrió un error al actualizar el perfil: ' + (error.message || 'Error desconocido'));
        });
    }
